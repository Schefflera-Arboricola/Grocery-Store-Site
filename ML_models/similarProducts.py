...
def similarProducts(given_product, products, categories,N=6):
    ...
    for product in products:
        ...
        product_description = 2*product['name'] + ' ' + 2* getCategoryDesc(product['category_id'], categories) + ' ' + product['description']
        product_descriptions.append(product_description)

    vectorizer = TfidfVectorizer()
    tfidf_matrix = vectorizer.fit_transform(product_descriptions)

    given_product_description = 2*given_product['name'] + ' ' + 2* getCategoryDesc(given_product['category_id'], categories) + ' ' + given_product['description']
