...
def require_login():
    ...
    if (
        not isinstance(current_user, DeliveryExecutive)
        or current_user.delivery_executive_id != delivery_executive_id
    ):
...
def dashboard(delexe_id):
    ...
    return render_template(
        "dashboard/dashboard_delexe.html",
        id=delexe_id,
        name=user.name,
        phone=user.phone_no,
    ...
    if request.method == "POST":
        ...
        if check_password_hash(user.password, password):
            ...
            if features != True:
                ...
                return render_template(
                    "editProfile/editProfile_delexe.html",
                    name=user.name,
                    phone=user.phone_no,
                )
            else:
...
@viewsDelExe.route(
    "/delexe/<int:delexe_id>/pendingDelieveries", methods=["GET", "POST"]
)
def pendingDelieveries(delexe_id):
    orders = OrderDetails.query.filter_by(
        delivery_executive_id=delexe_id, delivery_status="ORDER PLACED"
    ).all()
    return render_template(
        "userviews/delivery_executive/pending_deliveries.html",
        delexe_id=delexe_id,
        orders=orders,
    )
...
@viewsDelExe.route(
    "/delexe/<int:delexe_id>/completedDelieveries", methods=["GET", "POST"]
)
def completedDelieveries(delexe_id):
    orders = OrderDetails.query.filter_by(
        delivery_executive_id=delexe_id, delivery_status="DELIVERED"
    ).all()
    return render_template(
        "userviews/delivery_executive/completed_deliveries.html",
        delexe_id=delexe_id,
        orders=orders,
    )
...
@viewsDelExe.route(
    "/delexe/<int:delexe_id>/delivery_details/<int:order_id>", methods=["GET", "POST"]
)
def delivery_details(delexe_id, order_id):
    ...
    if request.method == "POST":
        ...
        if flag == "True":
            ...
            return render_template(
                "userviews/delivery_executive/delivery_details.html",
                delexe_id=delexe_id,
                order_id=order_id,
                customer=customer,
                products=products,
...
def sendOTP(otp, phone_no):
    ...
    message = client.messages.create(
        ...
        + str(
            phone_no
        ),  # this number should be verified if you are using a trail account
    )
    return None
